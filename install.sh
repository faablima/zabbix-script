...
function version_id_amazon()
{
    ...
    echo !! 2 !! OS Version determined as $d1  #prints os version id like this : VERSION_ID="7"

    d2=$(echo $d1 | cut -c13- | rev | cut -c2- |rev)
    #echo $d2        #prints os version id like this : 7.4

    d3=$(echo $d2 | awk '{print int($1)}')
    #echo $d3       #prints os version id like this : 7

    if [[ $d3 -eq 2 ]];     then rhel7
      else echo :-/ Failed at Step 2 : We"'"re Sorry. This script cannot be used for zabbix-agent installation on this machine && exit 0
    fi
}
...
#STEP 5
echo ========================================================================
echo STEP !! 5 !! Zabbix-Agent Installed
